...
class TestGetPdfOutlineHappyPath:
    ...
    @pytest.mark.asyncio
    async def test_simple_toc_formatted_as_markdown(self, monkeypatch, dummy_ctx, fake_zot):
        toc = [
            [1, "Introduction", 1],
            [1, "Methods", 5],
            [1, "Results", 10],
        ...
        result = await server.get_pdf_outline(item_key="PARENT01", ctx=dummy_ctx)
...
class TestNestedToc:
    ...
    @pytest.mark.asyncio
    async def test_indentation_by_level(self, monkeypatch, dummy_ctx, fake_zot):
        toc = [
            [1, "Chapter 1", 1],
            [2, "Section 1.1", 3],
        ...
        result = await server.get_pdf_outline(item_key="ITEM01", ctx=dummy_ctx)
...
class TestEmptyToc:
    ...
    @pytest.mark.asyncio
    async def test_empty_toc_message(self, monkeypatch, dummy_ctx, fake_zot):
        fake_zot._children["ITEM01"] = [_pdf_child()]
...
class TestNoPdfAttachment:
    ...
    @pytest.mark.asyncio
    async def test_no_children_at_all(self, monkeypatch, dummy_ctx, fake_zot):
    ...
    @pytest.mark.asyncio
    async def test_only_note_children(self, monkeypatch, dummy_ctx, fake_zot):
...
class TestMultipleChildrenOnlyPdfUsed:
    ...
    @pytest.mark.asyncio
    async def test_picks_pdf_among_mixed_children(self, monkeypatch, dummy_ctx, fake_zot):
        toc = [
            [1, "Abstract", 1],
            [1, "Body", 2],
        ]
    ...
    @pytest.mark.asyncio
    async def test_uses_first_pdf_when_multiple_pdfs(self, monkeypatch, dummy_ctx, fake_zot):
