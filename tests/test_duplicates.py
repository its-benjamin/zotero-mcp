...
class TestFindDuplicates:
    ...
    @pytest.mark.asyncio
    async def test_happy_path_title_grouping(self, monkeypatch, dummy_ctx):
        ...
        result = await server.find_duplicates(method="title", ctx=dummy_ctx)
    ...
    @pytest.mark.asyncio
    async def test_doi_matching(self, monkeypatch, dummy_ctx):
        ...
        result = await server.find_duplicates(method="doi", ctx=dummy_ctx)
    ...
    @pytest.mark.asyncio
    async def test_title_normalization(self, monkeypatch, dummy_ctx):
    ...
    @pytest.mark.asyncio
    async def test_no_duplicates_found(self, monkeypatch, dummy_ctx):
        ...
        result = await server.find_duplicates(method="both", ctx=dummy_ctx)

        assert "no duplicate" in result.lower() or "0" in result

    @pytest.mark.asyncio
    async def test_collection_scoping(self, monkeypatch, dummy_ctx):
        ...
        result = await server.find_duplicates(method="title", collection_key="COL1", ctx=dummy_ctx)
    ...
    @pytest.mark.asyncio
    async def test_large_library_cap(self, monkeypatch, dummy_ctx):
    ...
    @pytest.mark.asyncio
    async def test_both_method_combines_title_and_doi(self, monkeypatch, dummy_ctx):
...
class TestMergeDuplicatesDryRun:
    ...
    @pytest.mark.asyncio
    async def test_dry_run_returns_preview(self, monkeypatch, dummy_ctx):
        ...
        result = await server.merge_duplicates(keeper_key="KEEP", duplicate_keys=["DUP1"], confirm=False, ctx=dummy_ctx)
    ...
    @pytest.mark.asyncio
    async def test_dry_run_no_writes(self, monkeypatch, dummy_ctx):
        ...
        await server.merge_duplicates(keeper_key="KEEP", duplicate_keys=["DUP1", "DUP2"], confirm=False, ctx=dummy_ctx)
...
class TestMergeDuplicatesConfirm:
    ...
    @pytest.mark.asyncio
    async def test_tags_merged(self, monkeypatch, dummy_ctx):
        """All unique tags from duplicates are consolidated into keeper."""
        fake = self._setup_merge(monkeypatch)

        await server.merge_duplicates(keeper_key="KEEP", duplicate_keys=["DUP1", "DUP2"], confirm=True, ctx=dummy_ctx)
    ...
    @pytest.mark.asyncio
    async def test_children_reparented(self, monkeypatch, dummy_ctx):
    ...
    @pytest.mark.asyncio
    async def test_duplicates_trashed_not_deleted(self, monkeypatch, dummy_ctx):
    ...
    @pytest.mark.asyncio
    async def test_collections_consolidated(self, monkeypatch, dummy_ctx):
    ...
    @pytest.mark.asyncio
    async def test_keeper_in_duplicate_keys_removed_with_warning(self, monkeypatch, dummy_ctx):
        ...
        result = await server.merge_duplicates(
    ...
    @pytest.mark.asyncio
    async def test_empty_duplicate_list_error(self, monkeypatch, dummy_ctx):
        ...
        result = await server.merge_duplicates(keeper_key="KEEP", duplicate_keys=[], confirm=True, ctx=dummy_ctx)

        assert "error" in result.lower() or "no duplicate" in result.lower()
        assert fake.update_calls == []

    @pytest.mark.asyncio
    async def test_empty_after_keeper_removal_error(self, monkeypatch, dummy_ctx):
        ...
        result = await server.merge_duplicates(keeper_key="KEEP", duplicate_keys=["KEEP"], confirm=True, ctx=dummy_ctx)

        assert "no duplicate" in result.lower() or "empty" in result.lower() or "error" in result.lower()
        assert fake.update_calls == []

    @pytest.mark.asyncio
    async def test_partial_reparent_failure_aborts(self, monkeypatch, dummy_ctx):
        ...
        result = await server.merge_duplicates(keeper_key="KEEP", duplicate_keys=["DUP1"], confirm=True, ctx=dummy_ctx)
    ...
    @pytest.mark.asyncio
    async def test_version_refetch_after_operations(self, monkeypatch, dummy_ctx):
        ...
        await server.merge_duplicates(keeper_key="KEEP", duplicate_keys=["DUP1"], confirm=True, ctx=dummy_ctx)
    ...
    @pytest.mark.asyncio
    async def test_duplicate_keys_as_string_normalized(self, monkeypatch, dummy_ctx):
