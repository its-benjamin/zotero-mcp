...
def _search_single_page(
    ...
    if text_instances:
        rects, min_y, min_x = _convert_rects_to_zotero([r for r in text_instances], page)
        return {
            "pageIndex": page_index,
            "rects": rects,
            "sort_index": _build_sort_index(page_index, min_y, min_x),
...
def verify_pdf_attachment(pdf_path: str) -> bool:
    ...
    try:
        ...
        raise ImportError(
            ...
            'Install it with: pip install "zotero-mcp-server[pdf] @ git+https://github.com/its-benjamin/zotero-mcp.git@v0.3.1"'
        )
    try:
        doc = fitz.open(pdf_path)
        is_pdf = doc.is_pdf
